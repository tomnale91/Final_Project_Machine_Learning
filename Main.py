...
def runNeuralNetwork(X_train, X_val_and_test, y_train, Y_val_and_test, dataname):
    ...
    hist = NN_model.fit(X_train, y_train, epochs=1500, batch_size=9000, validation_data=(X_val, Y_val), verbose=0)
    ...
    plt.title('Neural Network - {0}\n Score = {1:.3f} '.format(str(dataname), test_acc))
    plt.scatter(Y_test, y_pred)
    plt.plot([Y_test.min(), Y_test.max()], [y_pred.min(), y_pred.max()], 'r', lw=2)
    plt.xlabel('Actual ')
    plt.ylabel('Predict')
    ...
    plt.title('Model loss - {} - {}'.format(dataname,Y_test))
    plt.ylabel('Loss')
    plt.xlabel('Epoch')
